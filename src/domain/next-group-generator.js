...
export default class NextGroupGenerator {
  ...
  _getRound = (roundId: string): Round => {
    const roundsWithId = this._tournament.rounds.filter(({ id }) => id === roundId)
  ...
  _hasPreviousRound = () => this._tournament.rounds[0].id !== this._round.id
  ...
  _generateGroup = (): ?DanceGroup => {
    ...
    if (this._removeUneven && pairs != null) {
      pairs = pairs.filter(({ leader, follower }) => leader != null && follower != null)
    }
  ...
  _getRemainingParticipants = (): Array<Participant> => {
    const alreadyDancedParticipants = new Set(this._getAlreadyCompetedParticipants())

    return this._roundParticipants.filter((par) => !alreadyDancedParticipants.has(par.id))
  }
  ...
  _makeEvenPairing = () => {
    ...
    while (this._isUnevenPairing(pairs) && this._allowReuseOfParticipants) {
      ...
      if (roleCounts.leader < roleCounts.follower) {
        remainingParticipants.push(this._getLeaderWithWorstFollower(remainingParticipants))
      } else {
        remainingParticipants.push(this._getFollowerWithWorstLeader(remainingParticipants))
      }
  ...
  _getFollowerWithWorstLeader = (excluding: Array<Participant>): Participant =>
    this._getParticipantWithWorstCounterpart('follower', excluding)

  _getLeaderWithWorstFollower = (excluding: Array<Participant>): Participant =>
    this._getParticipantWithWorstCounterpart('leader', excluding)
  ...
  _getParticipantWithWorstCounterpart = (
    ...
    for (const worstId of worstParticipants) {
      ...
      if (!excludeIds.has(counterPart) && !this._didDanceMoreThanOthers(counterPart)) {
        break
      }
    }
  ...
  _getParticipantWithId = (participantId: string): Participant => {
    const participantsWithId = this._roundParticipants.filter(({ id }) => id === participantId)
  ...
  _didDanceMoreThanOthers = (participantId: string) => {
    ...
    if (participantId in leaders) {
      return leaders[participantId] > Math.min(...Object.keys(leaders).map((id) => leaders[id]))
    }
    return followers[participantId] > Math.min(...Object.keys(followers).map((id) => followers[id]))
  }
