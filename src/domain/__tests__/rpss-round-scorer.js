...
describe('RPSS Round scorer', () => {
  ...
  const round2_2_2: Round = {
    ...
    groups: [
      {
        id: generateId(),
        pairs: [genPair(leaders, followers, 0, 0), genPair(leaders, followers, 1, 1)],
        dances: [genDance(dances[0].id), genDance(dances[1].id)],
      },
      {
        id: generateId(),
        pairs: [genPair(leaders, followers, 2, 2), genPair(leaders, followers, 3, 3)],
  ...
  test('[INTERNAL] _getParticipants', () => {
    ...
    const expected: Array<mixed> = [...leaders.map((l) => l.id), ...followers.map((f) => f.id)]
  ...
  test('[INTERNAL] _aggregateJudgeNote', () => {
    ...
    const result: Array<JudgeWeightedNote> = scorer._aggregateJudgeNote(scores, leaders[0].id)
  ...
  test('[INTERNAL] _computeJudgeWeightedNotes', () => {
    ...
    const result: Array<JudgeWeightedNote> = scorer._computeJudgeWeightedNotes(fullScaleScores)
    ...
    judges.forEach((judge) => {
      participants.forEach((participant) => {
        const r = result.filter((n) => n.judgeId === judge.id && n.participantId === participant.id)
  ...
  test('[INTERNAL] _genRankMatrixRow', () => {
    ...
    const result: RankMatrixRow = scorer._genRankMatrixRow(leaders[0].id, 4, 2, judgeRanks)

    const expected: RankMatrixRow = genRow(leaders[0], judgeRanks, [0, 1, 3, 4], 2, [0, 2, 8, 12])
  ...
  test('[INTERNAL] _genRankMatrixRow 2', () => {
    ...
    const expected: RankMatrixRow = genRow(leaders[0], judgeRanks, [0, 0, 0, 4], 3, [0, 0, 0, 16])
  ...
  test('[INTERNAL] _genRPSSRanking', () => {
    ...
    const resultLeader: Array<Score> = scorer._genRPSSRanking(judgeRanks, 'leader')

    const resultFollower: Array<Score> = scorer._genRPSSRanking(judgeRanks, 'follower')
