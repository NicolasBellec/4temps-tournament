...
export default class RoundScorer {
  ...
  scoreRound = (
    ...
    const danceScorer = new DanceScorer(this._judges, this._round.criteria, notes, {
      countPresident: this._countPresident,
      allowNegative: false,
    })
    const totals: { [id: string]: Array<number> } = this._initializeTotals(this._getParticipants())
  ...
  _initializeTotals = (participants: Array<string>): { [id: string]: Array<number> } =>
    participants.reduce((totals, id) => ({ ...totals, [id]: [0] }), {})

  _scoreFromDanceRule = (danceScores: Array<number>) => {
    if (this._round.multipleDanceScoringRule === 'average') {
      return (danceScores.reduce((acc, score) => acc + score, 0) * 1.0) / danceScores.length
    }
