...
export default class GroupGeneratorImpl implements GroupGenerator {
  ...
  _removeAbsentParticipants = () => {
    this._participants = new Set(Array.from(this._participants).filter((p) => p.isAttending))
  }
  ...
  _generateGroup = (): Array<Pair> => {
    ...
    while (this._participants.size > 0 && group.length < this._round.maxPairCountPerGroup) {
      // $FlowFixMe
  ...
  _getParticipantWithParticipantRole = (role: ParticipantRole): ?Participant => {
    switch (role) {
      case 'leader':
  ...
  _randomUntilParticipantRoleOrLeaderAndFollower = (role: ParticipantRole) => {
    ...
    do {
      participant = this._randomParticipant()
    } while (participant && participant.role !== role && participant.role !== 'leaderAndFollower')
