...
export default class RPSSRoundScorer {
  ...
  scoreRound(notes: Array<JudgeNote>): Array<Score> {
    ...
    */
    ...
    const ranking_follower: Array<Score> = this._genRPSSRanking(ranks, 'follower')

    return [...ranking_leader, ...ranking_follower]
  } // scoreRound

  _genRPSSRanking(ranks: Array<JudgeRank>, role: ParticipantRole): Array<Score> {
    /*
    ...
    */
    ...
    const rankMatrix: RankMatrix = this._genRankMatrix(participants, roleRanks, judgeMajority)

    const sortedRankMatrix: RankMatrix = rankMatrix.sort(this._RPSScompare.bind(this))

    const finalRank: Array<Score> = this._genScoresFromSortedRankMatrix(sortedRankMatrix, max_rank)

    return finalRank
  } // _genRPSSRanking

  _genScoresFromSortedRankMatrix(rm: RankMatrix, max_rank: number): Array<Score> {
  ...
  _RPSScompare(rowA: RankMatrixRow, rowB: RankMatrixRow): number {
    ...
    */
    ...
    for (let index = rowA.rankReachMajority + 1; index < rowA.row.length; index++) {
    ...
    positivJudges.forEach((judge) => {
      const rankA: ?JudgeRank = rowA.ranks.find((rank) => rank.judgeId === judge.id)
      const rankB: ?JudgeRank = rowB.ranks.find((rank) => rank.judgeId === judge.id)
  ...
  _genRankMatrixRow(
    ...
    */
    ...
    const votePerRank: Array<number> = [...Array(maxRank).keys()]
      ...
      .map((cur_rank) => {
        const votes: Array<JudgeRank> = participantRanks.filter((rank) => rank.rank === cur_rank)
        return votes.length
      })
  ...
  _genJudgeRanks(notes: Array<JudgeNote>): Array<JudgeRank> {
    ...
    */
    ...
    const weightedNotes: JudgeWeightedNote[] = this._computeJudgeWeightedNotes(notes)
    ...
    const judgeRanks: Array<JudgeRank> = this._judges
      ...
      .reduce((acc, judge) => {
        ...
        const leaderWeighted: Array<JudgeWeightedNote> = weighted.filter((note) => {
          const participant: ?string = leaderIds.find((leaderId) => leaderId === note.participantId)
          return participant != null
        })

        const followerWeighted: Array<JudgeWeightedNote> = weighted.filter((note) => {
        ...
        const leaderRanks: JudgeRank[] = this._genRanksFromWeightedScores(leaderWeighted)
        const followerRanks: JudgeRank[] = this._genRanksFromWeightedScores(followerWeighted)
  ...
  _genRanksFromWeightedScores(weightedScores: Array<JudgeWeightedNote>): Array<JudgeRank> {
    /*
    ...
    */
    ...
    const sorted: Array<JudgeWeightedNote> = weightedScores.sort((a, b) => b.score - a.score)
  ...
  _computeJudgeWeightedNotes(notes: Array<JudgeNote>): Array<JudgeWeightedNote> {
    ...
    */
    ...
    return participantIds.reduce((scoreAcc, participantId) => {
      ...
      // dance and per judge
      const scoresForParticipant: JudgeScore[] = danceIds.reduce((acc, danceId) => {
  ...
  _aggregateDanceNotes(
    ...
    */
    ...
    const aggregatedDanceNotes: Array<JudgeScore> = this._judges.reduce((acc, judge) => {
      ...
      const aggregatedJudgeNote: number = selectedNotes.reduce((acc, note) => acc + note.value, 0)
  ...
  _computeMultipleDanceScore(scoreJudge: Array<JudgeScore>): number {
    ...
    */
    ...
    if (this._round.multipleDanceScoringRule === 'average') {
      const score: number =
        (scoreJudge.reduce((acc, score) => acc + score.score, 0) * 1.0) / scoreJudge.length
  ...
  _isPositiveJudgeType(judgeType: JudgeType): boolean {
    return judgeType === 'normal' || (this._countPresident === true && judgeType === 'president')
