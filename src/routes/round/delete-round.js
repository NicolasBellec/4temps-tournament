...
class DeleteRoundRoute {
  ...
  route = async (req: ServerApiRequest, res: ServerApiResponse) => {
    const handler = new DeleteRoundRouteHandler(this._userId(req), this._tournamentRepository)

    try {
      handler.parseParams(req.params)
      if ((await handler.isUserAuthorized()) && !(await handler.isRoundStartedOrFinished())) {
