...
export default class SettleDrawRoute {
  ...
  route = async (req: ServerApiRequest, res: ServerApiResponse) => {
    ...
    const tournament: ?Tournament = await this._tournamentRepository.get(tournamentId)
    ...
    if (!tournament) {
      ...
      if (!activeRound) {
        ...
        !isTieBreakingJudge((req.session.user && req.session.user.id) || '', activeRound)
      ) {
        res.status(403)
        res.json({
      ...
      } else if (!scoreIncludesOnlyParticipantsOfRound(activeRound, roundScores)) {
        res.status(400)
        res.json({
        ...
        try {
          await this._tournamentRepository.updateRound(tournamentId, updatedRound)
...
function isTieBreakingJudge(judgeId: string, activeRound: Round): boolean {
  return activeRound.tieBreakerJudge != null && activeRound.tieBreakerJudge === judgeId
}

function scoreIncludesAllParticipants(round: Round, scores: Array<Score>): boolean {
...
function scoreIncludesOnlyParticipantsOfRound(round: Round, scores: Array<Score>): boolean {
  const participants = participantsInRound(round)
  return scores.map((score) => participants.includes(score.participantId)).every(Boolean)
}

function participantsInRound(round: Round): Array<string> {
  return pairsInRound(round)
    .reduce(
      (participants: Array<?string>, pair) => [...participants, pair.leader, pair.follower],
      []
    )
    .filter(Boolean)
}
