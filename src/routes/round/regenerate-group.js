...
export default function regenerateGroupRoute(
  ...
  return async (req: ServerApiRequest, res: ServerApiResponse) => {
    try {
      const tournament = await getTournament(tournamentRepository, req.params.tournamentId)
      ...
      if (!isGroupActive(group)) {
        ...
        generateGroups(tournament, round, await getNotesForRound(noteRepository, round))
...
function getRound(tournament: Tournament, id: string): Round {
  const round = tournament.rounds.reduce((res, round) => (round.id === id ? round : res), null)
...
function getGroup(round: Round, id: string): DanceGroup {
  const group = round.groups.reduce((res, group) => (group.id === id ? group : res), null)
...
function isGroupActive(group: DanceGroup): boolean {
  return group.dances.reduce((acc, { active, finished }) => acc || active || finished, false)
}
...
async function getNotesForRound(repo: NoteRepository, round: Round): Promise<Array<JudgeNote>> {
  const dances = round.groups.reduce((dances, group) => [...dances, ...group.dances], [])
...
function generateGroups(tournament: Tournament, round: Round, notes: Array<JudgeNote>) {
  let group = null
  do {
...
function handleError(e: mixed, res: ServerApiResponse) {
  if (e instanceof NoTournamentError || e instanceof NoRoundError || e instanceof NoGroupError) {
    res.sendStatus(404)
  } else {
    res.sendStatus(500)
  }
