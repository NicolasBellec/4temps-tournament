...
describe('/api/tournament/create', () => {
  ...
  test('Valid tournament returns 200 and the new tournamentid', async () => {
    expect(await createTournamentRoute(tournament.creatorId, tournament, repository)).toEqual({
      status: 200,
      body: tournament,
    })
  })
  ...
  test('Tournament is validated and returns status 400 when invalid', async () => {
    ...
    expect(
      await createTournamentRoute(tournament.creatorId, invalidTournament, repository)
    ).toEqual({
      status: 400,
      body: null,
    })
