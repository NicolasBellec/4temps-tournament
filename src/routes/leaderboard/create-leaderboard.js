...
function hasLost(participant: Participant, rounds: Array<LeaderboardRound>) {
  const losingFollowers = rounds.reduce((ack, r) => [...ack, ...r.losingFollowerScores], [])
  const losingLeaders = rounds.reduce((ack, r) => [...ack, ...r.losingLeaderScores], [])
  return (
...
function hydrateScores(roundScores: Array<Score>, participants: { [id: string]: Participant }) {
