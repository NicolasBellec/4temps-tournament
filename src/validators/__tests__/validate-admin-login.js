...
test('Returns valid admin if exists', async () => {
  ...
  const result = await validateAdminLogin(admin, () => new Promise((resolve) => resolve(fullAdmin)))
...
test('Returns null admin if not exists', async () => {
  ...
  const result = await validateAdminLogin(admin, () => new Promise((resolve) => resolve(null)))
