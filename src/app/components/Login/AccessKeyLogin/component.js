...
class Login extends PureComponent<Props, State> {
  ...
  render() {
    const { headerTitle, isLoading, isValidAccessKey, doesAccessKeyExist } = this.props
    ...
    return (
      <div styleName="center">
        <div styleName="width">
          ...
          <Form loading={isLoading} error={!isValidAccessKey || !doesAccessKeyExist}>
            <FormInput
              label="Access Key"
            ...
            />
            {!isValidAccessKey && <Message error content="Access keys are 10 characters long" />}
            <Button type="submit" onClick={this.onSubmit}>
              Submit
            </Button>
            {!doesAccessKeyExist && <Message error content="Access key does not exist!" />}
          </Form>
        </div>
      </div>
    )
