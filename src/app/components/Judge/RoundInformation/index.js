...
function mapStateToProps({ tournaments, rounds }: ReduxState): StateProps {
  const tournamentRounds: Array<Round> = tournaments.byId[tournaments.forJudge].rounds.map(
    (id) => rounds.byId[id]
  )
  const activeRound = getActiveRound(tournamentRounds)
  const activeGroupInformation = getActiveGroupInformation(activeRound)
  const activeDanceInformation = getActiveDanceInformation(activeGroupInformation.group)
  return {
...
const RoundInformationContainer = connect<Props, {}, StateProps, _, _, _>(mapStateToProps)(
  RoundInformation
)
