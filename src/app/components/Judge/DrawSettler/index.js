...
import type { OwnProps, Props, StateProps, DispatchProps } from './types'

type HydratedScore = { score: number, participant: Participant }

function mapStateToProps({ participants, tournaments, rounds, ui }: ReduxState): StateProps {
  ...
  const participantsOfRound = getParticipants(participants, tournaments.forJudge)
  const leaders = getParticipantsWithRole('leader', activeRound, participantsOfRound)
  const hydratedLeaderScores = hydrateScores(leaders, activeRound.roundScores)

  const followers = getParticipantsWithRole('follower', activeRound, participantsOfRound)
  const hydratedFollowerScores = hydrateScores(followers, activeRound.roundScores)

  return {
    isPairRound:
      tournament.rounds.indexOf(activeRound.id) === tournament.rounds.length - 1 ||
      tournament.type === 'classic',
    roundName: activeRound.name,
    passingCouplesCount: activeRound.passingCouplesCount,
    leaders: {
      winners: winningScores(activeRound.passingCouplesCount, hydratedLeaderScores),
    ...
    followers: {
      winners: winningScores(activeRound.passingCouplesCount, hydratedFollowerScores),
      draw: drawScores(activeRound.passingCouplesCount, hydratedFollowerScores),
      losers: loserScores(activeRound.passingCouplesCount, hydratedFollowerScores),
    },
    ...ui.settleDraw,
  }
}
...
function winningScores(passingCouplesCount: number, scores: Array<HydratedScore>) {
...
function loserScores(passingCouplesCount: number, scores: Array<HydratedScore>) {
  if (scores.length <= passingCouplesCount) {
    return []
  }
...
function hydrateScores(
  ...
  return roundScores
    .map((score) => {
      const participant = participants.find((participant) => participant.id === score.participantId)
      if (!participant) {
        return null
      }
