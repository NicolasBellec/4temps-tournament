...
import type { PairViewModel, StateProps, DispatchProps, Props, OwnProps } from './types'
...
function mapStateToProps(
  ...
  const pairs = getPairViewModels(round, participants, notes, isClassicTournament(tournaments))
...
function hasAllNotes(participantId: string, notes: NotesReduxState, criterionCount: number) {
  if (notes.byParticipant[participantId] == null) {
    return false
  }
  return Object.keys(notes.byParticipant[participantId]).length === criterionCount
}
