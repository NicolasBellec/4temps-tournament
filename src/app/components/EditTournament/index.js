...
class EditTournament extends Component<Props, State> {
  ...
  _renderMenuItem = (tab: TabName, content: string) => (
    <MenuItem key={tab} active={this.state.activeTab === tab} onClick={() => this._onClickTab(tab)}>
      {content}
    </MenuItem>
  )

  _renderGeneral = () => (
    <General tournamentId={this._getTournamentId()} history={this.props.history} />
  )

  _renderRounds = () => (
    <Rounds tournamentId={this._getTournamentId()} history={this.props.history} />
  )

  _renderJudges = () => <Judges tournamentId={this._getTournamentId()} />

  _renderAssistants = () => <Assistants tournamentId={this._getTournamentId()} />

  _renderParticipants = () => <Participants tournamentId={this._getTournamentId()} />

  _getTournamentId = (): string => this.props.match.params.tournamentId || ''
