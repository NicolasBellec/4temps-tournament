...
class RoundList extends Component<Props, State> {
    ...
    <TableRow key={round.id}>
      ...
      {!(round.active || round.finished) && (
        <TableCell textAlign="right">
          <Button size="tiny" floated="right" onClick={() => this.props.deleteRound(round.id)}>
            Delete
          </Button>
        </TableCell>
      )}
  ...
  hasActiveDance = (round: Round): boolean =>
    round.groups.reduce((acc, g) => [...acc, ...g.dances], []).filter((d) => d.active).length > 0

  getFinishedDances = (activeRound: Round) =>
    activeRound.groups.reduce((acc, g) => [...acc, ...g.dances], []).filter((d) => d.finished)
      .length

  getUnfinishedDances = (activeRound: Round) =>
    activeRound.groups.reduce((acc, g) => [...acc, ...g.dances], []).filter((d) => !d.finished)
      .length
  ...
  getUpcomingRounds = (): Round[] =>
    this.props.rounds.filter((r) => !(r.active || r.finished) && r.id != this.props.nextRound)
      ...
      <Container styleName="pad">
        ...
        {nextRound && (
          <Card>
            ...
            <Card.Content extra>
              <Button basic color="green" onClick={() => this.props.startRound(nextRound.id)}>
                Start Round
              </Button>
              <Button basic color="red" onClick={() => this.props.deleteRound(nextRound.id)}>
                Delete
              </Button>
            </Card.Content>
          </Card>
  ...
  render() {
    return (
      <Container>
        ...
        {this.hasNextRound() && !this.hasActiveRound() && this.renderNextRound()}
