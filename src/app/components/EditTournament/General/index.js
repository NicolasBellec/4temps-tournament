...
import type { Props, StateProps, DispatchProps, OwnProps } from './types'

function mapStateToProps(
  { tournaments, rounds, judges, participants, assistants, ui }: ReduxState,
  ...
  return {
    ...
    tournament: {
      ...
      rounds: (rounds.forTournament[tournamentId] || []).map((id) => rounds.byId[id]),
      judges: (judges.forTournament[tournamentId] || []).map((id) => judges.byId[id]),
...
const EditTournamentGeneralConnectedContainer = connect<Props, OwnProps, StateProps, _, _, _>(
  mapStateToProps,
  mapDispatchToProps
)(PreloadContainer)
