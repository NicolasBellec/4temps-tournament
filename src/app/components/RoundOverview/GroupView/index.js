...
function areAllGroupsGenerated(viewModel: ?RoundViewModel, participantCount: number): boolean {
  if (!viewModel) {
    return true
  }
  ...
  const viewModel: RoundViewModel = {
    ...
    groups: groups.map((g) => ({
      ...
      pairs: g.pairs.map((p, i) => ({
        ...
        follower: createParticipantViewModel(participants.byId[p.follower || '']),
      })),
...
function getActiveGroup(groups) {
  const activeGroups = [...Array(groups.length).keys()].filter((i) => {
    ...
    return group.dances.map((d) => !d.finished).reduce((ack, r) => ack || r, false)
  })
