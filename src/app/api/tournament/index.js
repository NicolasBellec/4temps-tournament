...
export const createTournamentApi = async (tournament: Tournament): Promise<Tournament> => {
  ...
  return apiPostRequest('/api/tournament/create', tournament, deserializeTournament)
}
...
export async function getTournamentsForUser(): promisedArray {
  const tournaments: Tournament[] = await apiGetRequest('/api/tournament/get', (tours) =>
    tours.map(deserializeTournament)
  )
...
export async function getAllTournaments(): promisedArray {
  const tournaments: Tournament[] = await apiGetRequest('/api/tournament/get/all', (tours) =>
    tours.map(deserializeTournament)
  )
  return normalizeTournamentArray(tournaments)
}
