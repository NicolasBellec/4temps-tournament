...
describe('Create round UI reducer', () => {
  test('Undefined returns default state', () => {
    ...
    ;[LIFECYCLE.START, LIFECYCLE.SUCCESS, LIFECYCLE.FAILURE].map((lifecycle) =>
      expect(reducer(undefined, makePackAction(lifecycle, 'INVALID_ACTION'))).toEqual(state)
    )
  })
  ...
  test('CREATE_ROUND action start sets flags', () => {
    ...
    expect(reducer(initial, makePackAction(LIFECYCLE.START, 'CREATE_ROUND'))).toEqual(expected)
  })
  ...
  test('CREATE_ROUND action success sets flags', () => {
    ...
    expect(reducer(initial, makePackAction(LIFECYCLE.SUCCESS, 'CREATE_ROUND'))).toEqual(expected)
  })
  ...
  test('CREATE_ROUND action failure sets flags and validation', () => {
    ...
    expect(reducer(initial, makePackAction(LIFECYCLE.FAILURE, 'CREATE_ROUND', payload))).toEqual(
      expected
    )
  })
})
