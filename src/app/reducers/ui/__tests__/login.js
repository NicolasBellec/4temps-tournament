...
test('When a login starts, isLoading is set to true', () => {
  expect(reducer(undefined, makePackAction(LIFECYCLE.START, 'LOGIN_USER'))).toMatchObject({
    isLoading: true,
  })
})

test('Only LOGIN_USER affects the state', () => {
  expect(reducer(undefined, makePackAction(LIFECYCLE.START, 'SOMETHING_ELSE'))).toMatchObject({
    isLoading: false,
  })
})
...
test('When a login is successful, isLoading is set to false', () => {
  ...
  expect(reducer(state, makePackAction(LIFECYCLE.SUCCESS, 'LOGIN_USER'))).toMatchObject({
    isLoading: false,
  })
})
...
test('When a login fails, isLoading is set to false', () => {
  ...
  expect(reducer(state, makePackAction(LIFECYCLE.FAILURE, 'LOGIN_USER'))).toMatchObject({
    isLoading: false,
  })
})
...
test('When a login is successful, the validation parameters are set', () => {
  ...
  expect(reducer(state, makePackAction(LIFECYCLE.SUCCESS, 'LOGIN_USER', payload))).toMatchObject(
    payload
  )
})
...
test('When a login fails, the validation parameters are set', () => {
  ...
  expect(reducer(state, makePackAction(LIFECYCLE.FAILURE, 'LOGIN_USER', payload))).toMatchObject(
    payload
  )
})
