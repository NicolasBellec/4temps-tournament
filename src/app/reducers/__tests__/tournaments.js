...
describe('Tournament reducer', () => {
  test('Default value is no tournaments', () => {
    ...
    expect(reducer(undefined, makePackAction(LIFECYCLE.FAILURE, 'INVALID_ACTION'))).toEqual(
      getInitialState()
    )
  })

  test('Invalid action does not change state', () => {
    expect(reducer(getInitialState(), makePackAction(LIFECYCLE.START, 'INVALID_ACTION'))).toEqual(
      getInitialState()
    )
    expect(reducer(getInitialState(), makePackAction(LIFECYCLE.SUCCESS, 'INVALID_ACTION'))).toEqual(
      getInitialState()
    )
    expect(reducer(getInitialState(), makePackAction(LIFECYCLE.FAILURE, 'INVALID_ACTION'))).toEqual(
      getInitialState()
    )
  })
  ...
  test('GET_ALL_TOURNAMENTS start sets isLoading to true', () => {
    expect(
      reducer(getInitialState(), makePackAction(LIFECYCLE.START, 'GET_ALL_TOURNAMENTS'))
  ...
  test('GET_ALL_TOURNAMENTS failure sets isLoading to false and' + 'isInvalidated to true', () => {
    expect(
      reducer(getInitialState(), makePackAction(LIFECYCLE.FAILURE, 'GET_ALL_TOURNAMENTS'))
    ).toEqual({
      ...getInitialState(),
      isLoading: false,
      isInvalidated: false,
  ...
  test('GET_ADMIN_TOURNAMENTS start sets loading to true', () => {
    expect(
      reducer(getInitialState(), makePackAction(LIFECYCLE.START, 'GET_ADMIN_TOURNAMENTS'))
    ).toEqual({ ...getInitialState(), isLoading: true })
  })
  ...
  test('CREATE_TOURNAMENT success adds the new tournament', () => {
    ...
    expect(reducer(state, makePackAction(LIFECYCLE.SUCCESS, 'CREATE_TOURNAMENT', payload))).toEqual(
      {
        ...state,
        allIds,
        byId,
  ...
  test('EDIT_TOURNAMENT success sets the new tournament', () => {
    ...
    expect(reducer(state, makePackAction(LIFECYCLE.SUCCESS, 'EDIT_TOURNAMENT', payload))).toEqual({
      ...state,
      byId: {
  ...
  test('LOGOUT_USER success resets admin tournaments', () => {
    ...
    expect(reducer(state, makePackAction(LIFECYCLE.SUCCESS, 'LOGOUT_USER'))).toEqual(
      getInitialState()
    )
  })
})
