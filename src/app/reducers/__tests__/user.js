...
describe('User reducer', () => {
  ...
  test('Default id is empty string', () => {
    expect(reducer(undefined, makePackAction(LIFECYCLE.SUCCESS, 'INVALID_ACTION'))).toEqual({
      id: '',
      role: '',
      tournamentId: '',
    })
  ...
  test('Successful login sets id', () => {
    ...
    expect(
      reducer(initialState, makePackAction(LIFECYCLE.SUCCESS, 'LOGIN_USER', payload))
    ).toEqual({ id, role: 'admin', tournamentId: '' })
  })

  test('Failed login sets userId to previous value', () => {
    expect(reducer(initialState, makePackAction(LIFECYCLE.FAILURE, 'LOGIN_USER'))).toEqual(
      initialState
    )
